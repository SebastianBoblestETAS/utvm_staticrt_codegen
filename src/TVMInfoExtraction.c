...
void *create_tvm_rt(const char *json_data, const char *params_data,
                    uint64_t params_size) {
  return (TVMGraphRuntime *)tvm_runtime_create(json_data, params_data,
                                               params_size, NULL);
}
...
static uint32_t *GetStaticInputEIDs(size_t *numReturned, TVMGraphRuntime *g,
...
static Storage_Info *GetOrAddStorage(Graph_Info *gi, void *p, size_t sz,
                                     bool copy_data) {
  ...
  gi->numStorages++;
  gi->storages =
      realloc(gi->storages, gi->numStorages * sizeof(Storage_Info *));
...
Graph_Info *extract_graph_info(void *grt, const char *params_data,
                               uint64_t params_size) {
  ...
  uint32_t *staticInputEIDs =
      GetStaticInputEIDs(&numStaticInputEIDs, g, params.data, params.size);
